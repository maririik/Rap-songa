...
if search_query:
    ...
    if not song_data.empty:
        ...
        for index, song in song_data.iterrows():
            ...
            sns.boxplot(data=df['sentiment_score'], ax=axes[0], color="skyblue")
            axes[0].scatter(
                [0], song['sentiment_score'], color='red', s=100, label='Selected Song'
            )
            axes[0].set_title("Sentiment Score Distribution")
            axes[0].legend()

            # Box plot for profanity percentage
            sns.boxplot(data=df['profanity weighting (%)'], ax=axes[1], color="lightgreen")
            axes[1].scatter(
                [0], song['profanity weighting (%)'], color='red', s=100, label='Selected Song'
            )
            axes[1].set_title("Profanity Percentage Distribution")
            axes[1].legend()
